...
This is a soft delete module of django, quoted from the scoursen/django-softdelete
project, and may be updated or modified later.
"""
    ...
    try:
        ...
        try:
            ...
            if create:
                ...
                logger.debug("Raising ObjectDoesNotExist")
...
class SoftDeleteObject(models.Model):
    ...
    def delete(self, *args, **kwargs):
        if self.deleted_at:
            logger.debug("HARD DELETEING type %s, %s", type(self), self)
            try:
            ...
            pre_soft_delete.send(sender=self.__class__,
                                 instance=self,
                                 using=using)
            logger.debug('SOFT DELETING type: %s, %s', type(self), self)
            cs = kwargs.get('changeset') or _determine_change_set(self)
            ...
            logger.debug("FINISHED SOFT DELETING RELATED %s", self)
    ...
    def undelete(self, using='default', *args, **kwargs):
        ...
        cs.undelete(using)
        logger.debug('FINISHED UNDELETING RELATED %s', self)
...
This is the basic model for all apps
"""
